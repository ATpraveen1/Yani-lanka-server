...
exports.GetDashBoardData = async (req, res) => {
    ...
    try {
        // Aggregate billing data and join with produced inventory
        let data = await Billings.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    latestBills: { $push: "$$ROOT" },
                    totalChequeAmount: { $sum: "$CheckAmount" }
                }
            },
            {
                $addFields: {
                    latestBills: { $slice: ["$latestBills", -10] },
...
exports.GetFilteredProducts = async (req, res) => {
  ...
  if (req.body.productName) {
      filter.ProductName = { $regex: req.body.productName, $options: 'i' }; // Case-insensitive search
  }

  try {
      let products = await ProducedInventory.find(filter);
